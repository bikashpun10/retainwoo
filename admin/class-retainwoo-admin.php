...
class RetainWoo_Admin {
    ...
    public static function menu() {
        add_menu_page( __( 'RetainWoo', 'retainwoo' ), __( 'RetainWoo', 'retainwoo' ), 'manage_woocommerce', 'retainwoo',
            [ __CLASS__, 'dashboard' ], 'dashicons-shield-alt', 58 );
        add_submenu_page( 'retainwoo', __( 'Dashboard', 'retainwoo' ), __( 'Dashboard', 'retainwoo' ), 'manage_woocommerce', 'retainwoo', [ __CLASS__, 'dashboard' ] );
        add_submenu_page( 'retainwoo', __( 'Settings', 'retainwoo' ), __( 'Settings', 'retainwoo' ), 'manage_woocommerce', 'retainwoo-settings', [ __CLASS__, 'settings' ] );
    }
...
public static function sanitize_option( $value ) {
    ...
    public static function enqueue( $hook ) {
        if ( strpos( $hook, 'retainwoo' ) === false ) return;
        // shared tokens first
        wp_enqueue_style( 'retainwoo-common', RETAINWOO_URL . 'assets/css/retainwoo-common.css', [], RETAINWOO_VERSION );
        wp_enqueue_style( 'retainwoo-admin', RETAINWOO_URL . 'assets/css/retainwoo-admin.css', [], RETAINWOO_VERSION );
    }

    private static function nav( $current ) {
        $pages = [
            'retainwoo'          => [ 'icon' => '📊', 'label' => __( 'Dashboard', 'retainwoo' ) ],
            'retainwoo-settings' => [ 'icon' => '⚙️', 'label' => __( 'Settings', 'retainwoo' ) ],
        ];
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td><span class="cs-pill"><?php printf( esc_html__( '%s times', 'retainwoo' ), esc_html( $row->cnt ) ); ?></span></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
